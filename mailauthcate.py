...
def authenticate():
    ...
    if not creds or not creds.valid:
        ...
        server = Thread(target=mailapp.run, kwargs={'port': 5000})
        server.start()
...
def google_auth_callback():
    ...
    flow = Flow.from_client_secrets_file('credentials.json', scopes=SCOPES, redirect_uri='http://localhost:5000/callback')
